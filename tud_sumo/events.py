        ...
        if isinstance(events, Event): self.scheduled_events[events.id] = events
            ...
            if isinstance(events, dict):
                for event_id, event_params in events.items():
                    ...
                    else: raise TypeError("EventScheduler.init: Invalid event_params dict (must contain [dict|Event], not '{0}').".format(type(event_params).__name__))
                ...
                if events.endswith(".json"):
                    ...
                    else: raise FileNotFoundError("EventScheduler.init: Event parameters file '{0}' not found.".format(events))
                else: raise ValueError("EventScheduler.init: Event junction parameters file '{0}' (must be '.json' file).")
                ...
                raise TypeError("EventScheduler.init: Invalid event_params (must be [dict|filepath|(Event)], not '{0}').".format(type(events).__name__))
...
class Event:
    def __init__(self, event_id, event_params, sim):
        ...
        if not isinstance(event_params, dict) and not isinstance(event_params, str):
            raise TypeError("Event.init: Invalid event_params (must be [dict|filepath (str)], not '{0}').".format(type(event_params).__name__))
            ...
            if event_params.endswith(".json"):
                ...
                else: raise FileNotFoundError("Event.init: Event parameters file '{0}' not found.".format(event_params))
            else: raise ValueError("Event.init: Event junction parameters file '{0}' (must be '.json' file).")

        if "start_time" in event_params.keys(): self.start_time = event_params["start_time"]
        else: raise KeyError("Event.init: Event 'start_time' is not given and is required.")
        ...
        if "end_time" in event_params.keys():
            ...
            raise KeyError("Event.init: Event 'end_time' and 'event_duration is not given and 1 is required.")

        if "edge" not in event_params.keys() and "vehicle" not in event_params.keys():
            raise KeyError("Event.init: Neither 'edge' or 'vehicle' parameters are given and one or both is required.")
        
        if "edge" in event_params.keys():
            if "actions" not in event_params["edge"].keys() and "egde_ids" not in event_params["edge"].keys():
                raise KeyError("Event.init: Edge events require 'actions' and 'edge_ids' parameters.")
        ...
        if "vehicle" in event_params.keys():
            if "actions" not in event_params["vehicle"].keys() and "locations" not in event_params["vehicle"].keys():
                raise KeyError("Event.init: Vehicle events require 'actions' and 'locations' parameters.")
        ...
        if self.v_actions != None:
            if self.end_time > self.sim.curr_step:
                ...
                for location in self.locations:
                    ...
                    else: raise KeyError("EventScheduler.update_events: Object '{0}' has unknown type (must be detector or geometry ID).".format(location))
